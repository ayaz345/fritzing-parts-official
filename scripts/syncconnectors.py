...
def main():
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:], "ho:n:c:a", [
                                   "help", "old", "new", "csv", "append"])
    ...
    csvfile.write("{0} => {1},,,,,\n".format(
        os.path.basename(oldfn), os.path.basename(newfn)))
    ...
    for oldix in range(0, len(oldelements)):
        ...
        oldname = oldelement.attrib.get("name")
        if oldmarker == None:
            csvfile.write(formatstring.format(
                oldid=oldid, oldname=oldname, action="not found", newid="", newname=""))
            continue
        ...
        if oldname.lower() == newname.lower():
            csvfile.write(formatstring.format(
                oldid=oldid, oldname=oldname, action="same", newid=newid, newname=newname))
            continue

        csvfile.write(formatstring.format(oldid=oldid, oldname=oldname,
                                          action="replacedby", newid=newid, newname=newname))
    ...
    for newix in range(0, len(newelements)):
        ...
        if newmarker == None:
            csvfile.write(formatstring.format(oldid="", oldname="",
                                              action="not found", newid=newid, newname=newname))
